...
def write_entities(input_folder, output_folder, binary_folder=None):
    # Delete all files in the folder initially to prevent appending
    ext = ".ann"
    filelist_output = [f for f in os.listdir(output_folder) if f.endswith(ext)]
    for f in filelist_output:
        os.remove(os.path.join(output_folder, f))
    filelist_binary = [f for f in os.listdir(binary_folder) if f.endswith(ext)]
    for f in filelist_binary:
        os.remove(os.path.join(binary_folder, f))

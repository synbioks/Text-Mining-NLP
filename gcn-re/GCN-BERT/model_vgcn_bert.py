# Original by Lu, et al.-- https://github.com/Louis-udm/VGCN-BERT
...
class Pretrain_VGCN(nn.Module):
    ...
    def __init__(self, word_emb, word_emb_dim, gcn_adj_dim, gcn_adj_num, gcn_embedding_dim, num_labels,
                 dropout_rate=0.2):
    ...
    def forward(self, vocab_adj_list, gcn_swop_eye, input_ids, token_type_ids=None, attention_mask=None):
        ...
        if self.gcn_embedding_dim > 0:
            ...
            for i in range(self.gcn_embedding_dim):
                tmp_pos = (attention_mask.sum(-1) - 2 - self.gcn_embedding_dim + 1 + i) + torch.arange(0,
                                                                                                       input_ids.shape[
                                                                                                           0]).to(
                    input_ids.device) * input_ids.shape[1]
...
class VGCN_Bert(BertModel):
    ...
    def __init__(self, config, gcn_adj_dim, gcn_adj_num, gcn_embedding_dim, num_labels, output_attentions=False,

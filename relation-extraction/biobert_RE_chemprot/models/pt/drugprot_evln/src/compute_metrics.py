...
def main(y_true, y_pred, reltype2tag, gs_rel_list, pred_rel_list, splitname):
    '''
    ...
    for k,v in reltype2tag.items():
        ...
        print(splitname + f"  p_{k}={round(P, 3)} \t r_{k}={round(R, 3)} \t f1_{k}={round(F1, 3)}")
    ...
    print(splitname + f"  p_micro={round(P, 3)} \t r_micro={round(R, 3)} \t f1_micro={round(F1, 3)}")
    return F1

...
class EndToEnd(nn.Module):
    ...
    def __init__(self, state_path, top_model):
        ...
        self.bert = BertModel.from_pretrained(state_path).to("hpu")
        self.top_model = top_model.to("hpu")

    def forward(self, x):
        y = self.bert(**x, return_dict=True).last_hidden_state #768
        y = y[:, 0, :]
        y = self.top_model(y) #final
        return y
    ...
    def my_predict(self, x, return_score=False):
        x = {k: v.to("hpu") for k, v in x.items()}

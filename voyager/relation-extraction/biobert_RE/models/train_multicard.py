...
# config_file_path = '/home/jbentley2/.config/config.json'
# # Read API key from config file
# with open(config_file_path, 'r') as file:
#     config = json.load(file)
#     api_key = config['weights_and_biases']['api_key']
...
def train_net(task_name, net, train_dataloader, valid_dataloader, test_dataloader, loss_fn, optimizer, args):
    ...
    if args.record_wandb:
        ...
        # name=args.record_wandb
        wb_run = wandb.init(project='RE', name='multicard_10_epochs', entity="ucsd_sbks", config = args)
    ...
    for pub_num in tqdm(os.listdir(args.inference_data)): # find all data folder in the dataset directory
        ...
        with torch.no_grad():
            for _, (id1s, id2s, batch_x) in enumerate(dataloader):
                pred, score = net.my_predict(batch_x, return_score=True)

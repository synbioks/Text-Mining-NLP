...
def train_net(task_name, net, train_dataloader, valid_dataloader, test_dataloader, loss_fn, optimizer, args):
    ...
    for epoch_count in range(args.epochs):
        ...
        for i, (x, y) in enumerate(tqdm(train_dataloader)):
            ...
            #PFR added to fix 'unsed param issue', see also DDP  parameters in model=DDP(...)
            #net.requires_grad_(True)  #or bert_required(Ture) or top_model.req..(T..)?

            # net.train_step(x, y, loss_fn, optimizer)
            x = {k: v.to("hpu") for k, v in x.items()}
            y = y.to("hpu")
            out = net(x)
            loss = loss_fn(out, y)
            loss.backward()
            htcore.mark_step()    # added for multinode 
            ...
            #PFR not sure if grad accum works with DDP
            if 1:  #PFR hard coded  1  sample_counter >= args.ga_batch_size:
                optimizer.step()
                #htcore.mark_step()     #PFR not sure we need both mark-steps 
                optimizer.zero_grad()
                sample_counter = 0
                train_step_count += 1
...
def test_net(task_name, net, test_dataloader, limit=None):
    ...
    with torch.no_grad():
        for i, (x_batch, y_batch) in enumerate(tqdm(test_dataloader)):
            ...
            #PFR changed predict to new name and used net.module,b/c of DDP wrapper 
            #NOTE, probably all calls to predict should be set up like this
            if do_mnode:
                pred = net.module.my_predict(x_batch).cpu().numpy() 
            else:
                pred = net.my_predict(x_batch).cpu().numpy()
            print("Predicted",type(pred),pred.shape)  #numpy array
            print(pred[0:5]) #PFR so its not a class output but real numbers
            ...
            x = {k: v.to("hpu") for k, v in x_batch.items()}
            out = net(x) 
            y = y_batch.to("hpu")
            loss = loss_fn(out, y)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bert-state-path', type=str, default='dmis-lab/biobert-large-cased-v1.1')
    ...
    parser.add_argument('--train-data', type=str, default='data/training/train.txt')
    parser.add_argument('--valid-data', type=str, default='data/training/vali.txt')
    parser.add_argument('--test-data', type=str, default='data/dev/merged.txt')
    parser.add_argument('--label-map-name', type=str, default='merged')
    parser.add_argument('--inference-data', type=str, default=None) #ACS here 
    ...
    parser.add_argument('--record-wandb', type=bool, default=False)
    args = parser.parse_args()
    # print out all package versions and named
#PFR took out    print('Installed packages:')
#    print(os.system('pip list'))

#---------- PFR -----------------------------------------------------
#  set up hccl backend, distoptimizer, DDP for model, and dataloader with distrib/sampling
#     train w/mark step
# ----------------------------------------------------------------------
    os.environ["PT_HPU_LAZY_MODE"]="1"
    do_mnode=1   #also set dataloader domnode
    if do_mnode: #turn off for single node jobs
      try:
            ...
            print('INFO, mpicomm rk:',rank,' sz:',size)
            if size > 1:
                args.rank = rank
                ...
                print('INFO, sz>1, port:',os.getenv('MASTER_PORT'),' address:',os.getenv('MASTER_ADDR'))

                #os.environ["ID"] = str(args.rank % 8 )
                #not used currently 
                #os.environ["LOCAL_RANK"] = str(args.rank % 8)
                import habana_frameworks.torch.distributed.hccl
                import torch.distributed as dist
                dist.init_process_group('hccl', rank=args.rank, world_size=args.world_size)
            else:
                print('INFO, util, init notdistmode, size not >1, set arg dist to F')
                args.distributed = False
                
      except Exception as e:
            print(e)
            print("INFO, util,excptn, mpi4py get comm, so setting arg dist to F,will not run distributed mode")
            exit() #not return
# ---------------------------------------------------------------------
    ...
    net = net.to('hpu')   #PFR note, this added too 
...
#            dataset2)
    if do_mnode:
       net = torch.nn.parallel.DistributedDataParallel(net, bucket_cap_mb=100, broadcast_buffers=False,
